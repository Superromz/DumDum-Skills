...
List the features, pages, or components that are complete and working. Describe each one in human terms, describing what it does, not how it's coded.
...
> - "The settings file references a database connection, but the connection details are blank, so it won't be able to save data until that's filled in"
